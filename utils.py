...
def count_reset():
    df=pd.read_csv(path/'voters_List_database.csv')
    df=df[['voter_id','Name','Gender','Zone','City','Passw','hasVoted']]
    for index, row in df.iterrows():
        df['hasVoted'].iloc[index]=0
    df.to_csv(path/'voters_List_database.csv')

    df=pd.read_csv(path/'candidadtes_List_database.csv')
    df=df[['Sign','Name','Vote Count']]
    for index, row in df.iterrows():
        df['Vote Count'].iloc[index]=0
    df.to_csv (path/'candidadtes_List_database.csv')
